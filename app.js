...
const APP_VERSION = "BFAHK-20260701-daily-schedule-v28";
...
function renderDailySchedule(sessions) {
  ...
  const dayEnd = 23 * 60;
  if (!sessions.length) {
    return `<div class="daily-schedule-gap is-full"><strong>07:00-23:00</strong><span>全日暫未有課堂</span></div>`;
  }
  ...
  sessions.forEach((session) => {
    ...
    if (start > cursor) items.push(dailyScheduleGap(cursor, start));
    items.push(timetableSessionRow(session, false));
    cursor = Math.max(cursor, end);
  });
  if (cursor < dayEnd) items.push(dailyScheduleGap(cursor, dayEnd));
  return items.join("");
}

function dailyScheduleGap(start, end) {
  const minutes = end - start;
  if (minutes < 15) return "";
  return `
    <div class="daily-schedule-gap">
      <strong>${formatMinutesAsTime(start)}-${formatMinutesAsTime(end)}</strong>
      <span>空檔 ${formatDurationShort(minutes)}</span>
    </div>
  `;
}
    ...
    <article class="session-row timetable-session-row ${classType}">
      <div class="session-main">
        <div class="timetable-session-title">
          ...
          <h4>${escapeHtml(dateLabel)}${escapeHtml(formatSessionTimeRange(session))} · ${escapeHtml(session.groupName)}</h4>
        </div>
        <div class="meta-line">
    ...
    <article class="attendance-card">
      <header class="attendance-card-header">
        <div class="attendance-title-block">
          <h4>${escapeHtml(formatSessionTimeRange(session))} · ${escapeHtml(session.groupName)}</h4>
          <div class="meta-line">
  ...
  document.getElementById("paymentHistoryList").innerHTML = payments.length
    ? payments.map((payment) => `
        <article class="payment-history-row">
          <div><strong>${escapeHtml(payment.packageName)}</strong><span>${escapeHtml(formatDisplayDate(payment.date))}</span></div>
          <div><strong>${formatMoney(payment.amount)}</strong><span>${escapeHtml(payment.method || "未註明")}</span></div>
          <div><span>${Number(payment.credits || 0)} credit</span>${payment.note ? `<span>${escapeHtml(payment.note)}</span>` : ""}</div>
        </article>
      `).join("")
