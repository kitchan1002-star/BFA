...
const APP_VERSION = "BFAHK-20260701-coach-schedule-v26";
...
function splitAllowedGroups(value) {
  return String(value || "")
    .split(/[,，、;\n|]+/)
...
function canAccessGroup(groupName, account = currentAccount()) {
  if (!isCoachRole()) return true;
  const groups = splitAllowedGroups(account?.allowedGroups || account?.allowed_groups || "");
  if (!groups.length) return false;
  if (groups.includes("all") || groups.includes("*")) return true;
  return groups.includes(normalizeGroupKey(groupName));
}

function canAccessSession(session) {
  if (!isAuthenticated()) return false;
  if (!canAccessGroup(session?.groupName)) return false;
  if (!isCoachRole()) return true;
  const coachName = normalizeCoachKey(currentAccount()?.coachName || currentAccount()?.coach_name || "");
  return Boolean(coachName && coachName === normalizeCoachKey(session?.coach));
}
...
function wireForms() {
  ...
  document.getElementById("sessionForm").addEventListener("submit", (event) => {
    ...
    document.getElementById(classType === "individual" ? "individualAttendanceDate" : "attendanceDate").value = session.date;
    form.reset();
...
function clearProtectedViews(message = "請先登入帳戶") {
  ...
  document.getElementById("recentSessionList").innerHTML = `<div class="empty-state">${escapeHtml(message)}</div>`;
...
function renderDashboard() {
  const today = hkTodayISO();
  const nextWeek = addDaysISO(today, 7);
  const previousWeek = addDaysISO(today, -7);
  ...
  const recentSessions = visibleSessions
    .filter((session) => session.date < today && session.date >= previousWeek)
  ...
  document.getElementById("todaySessionList").innerHTML = sessionsToday.length
    ? sessionsToday.map((session) => timetableSessionRow(session, false)).join("")
  ...
  document.getElementById("recentSessionList").innerHTML = recentSessions.length
    ? recentSessions.map((session) => timetableSessionRow(session, true)).join("")
    : `<div class="empty-state">最近 7 日未有課堂</div>`;
...
function scheduleSessionsForCurrentView() {
  ...
  return sessions.filter((session) => normalizeCoachKey(session.coach) === coachName);
}
...
function configureScheduleForm(classType = "group") {
  ...
  if (typeSelect) typeSelect.value = classType === "individual" ? "individual" : "group";
  if (groupName) groupName.placeholder = classType === "individual" ? "例：私人訓練／學員姓名" : "例：U12 小組A";
