...
const APP_VERSION = "BFAHK-20260628-class-modules-v20";
...
async function loginRequest(accountId, password) {
  try {
    return await cloudFormPostRequest("login", { accountId, password }, null, { timeoutMs: 30000 });
  } catch (error) {
    if (!String(error?.message || error).includes("逾時")) throw error;
...
function canAccessSession(session) {
  ...
  return canAccessGroup(session?.groupName);
}

function canAccessView(view) {
  if (view === "auth") return true;
  if (!isAuthenticated()) return !isProtectedView(view);
  if (isCoachRole()) return view === "attendance" || view === "individual";
  return true;
}
...
function renderAll() {
  ...
  if (isAuthenticated()) {
    if (isCoachRole()) {
      clearProtectedViews("此帳戶只可使用課堂點名");
    } else {
      renderDashboard();
      renderStudents();
      renderRevenue();
